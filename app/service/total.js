...
class Total extends Service {
  async get() {
    ...
    //   LIMIT 0, 200';
    let models = this.app.model.sequelize;
  ...
  async getTotalInfo() {
    let row=await this.app.model.Total.find({where: { name: 'total' }});
    if(!row){
        row=await this.app.model.Total.create({name:'total'});
    }
  ...
  async plusDeleteModule() {
    let models = this.app.model.sequelize;
    var sql =
      "UPDATE total SET module_delete=module_delete+1 WHERE name='total'";
